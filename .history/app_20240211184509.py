from flask import Flask, render_template, request, redirect, url_for, has_app_context
...
@app.handle_user_exceptionapp_context()
...
def google_callback():
    ...
    if id_info:
        ...
        if not user:
            # Create new user
            user = User(google_id=id_info['sub'], username=id_info['email'])
            db.session.add(user)
            db.session.commit()
        login_user(user)
        return redirect(url_for('addview'))
